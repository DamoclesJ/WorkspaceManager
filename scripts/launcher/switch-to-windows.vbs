...
WriteLog "INFO", "Windows", "Launcher started"

If Not fileSystem.FileExists(healthScriptPath) Then
    WriteLog "ERROR", "Windows", "Health check script not found: " & healthScriptPath
...
If Not fileSystem.FileExists(scriptPath) Then
    WriteLog "ERROR", "Windows", "Switch script not found: " & scriptPath
...
If healthExitCode <> 0 Then
    WriteLog "ERROR", "Windows", "Health check failed with exit code " & healthExitCode
...
WriteLog "INFO", "Windows", "Health check passed"
command = "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File """ & scriptPath & """"
WriteLog "INFO", "Windows", "Switch script launch requested"
